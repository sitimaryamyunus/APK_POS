...
class ItemPenjualanController extends Controller
{
    ...
    {
        // PERBAIKAN: Mencari data lewat ID manual untuk membongkar error 403 Policy
        ...
        DB::transaction(function () use ($itempenjualan) {
            ...
            // 🔼 Kembalikan stok produk jika data produk tersedia
            if ($produk) {
                $produk->increment('stok', $itempenjualan->kuantitas);
            }

            // ❌ Hapus item dari nota/keranjang belanja
            $itempenjualan->delete();

            // 🔄 Hitung ulang akumulasi total nota belanja terbaru
            if ($sale) {
                $sale->update([
