        ...
        <div class="card-body">
            <form action="{{ route('auth') }}" method="POST">
                ...
                <div class="mb-3 text-start">
                    ...
                    @error('email')
                    <div class="custom-error-badge" style="margin-top: 5px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <button type="submit" class="btn btn-login" id="btnLogin">Masuk</button>
            </form>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const errorBadges = document.querySelectorAll('.custom-error-badge');

    errorBadges.forEach(function(badge) {
        const textMessage = badge.innerText;
        const match = textMessage.match(/(\d+)\s*detik/);
        
        if (match) {
            let sisaDetik = parseInt(match[1], 10);
            
            // Kunci tombol masuk
            btnLogin.disabled = true;
            btnLogin.style.opacity = '0.6';
            btnLogin.style.cursor = 'not-allowed';
            btnLogin.innerText = `Tunggu (${sisaDetik}s)`;

            const timer = setInterval(function() {
                sisaDetik--;
                btnLogin.innerText = `Tunggu (${sisaDetik}s)`;
                
                // Mengambil pesan error depan (Email salah/Kata sandi salah) agar tidak hilang tertimpa
                const kataPembuka = textMessage.split("Terlalu banyak")[0];
                badge.innerText = `${kataPembuka}Terlalu banyak percobaan login. Silakan coba lagi dalam ${sisaDetik} detik.`;
